<script>
	...
	async function getParts({ fetch }) {
		const [issueData, issueBurndown, sawData, sawBurndown] = await Promise.all(
			[
		...
		if (issueData.ok && issueBurndown.ok && sawData.ok && sawBurndown.ok) {
