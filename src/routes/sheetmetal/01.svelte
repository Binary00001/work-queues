<script>
	...
	async function getParts() {
		const [
			...
			[
				fetch('http://imaginetics193.imagineticsinc.local:4004/api/testing/dept/num/0900'),
				// fetch('/production/burndown/0300.json'),
				fetch('http://imaginetics193.imagineticsinc.local:4004/api/testing/dept/num/1000')
		...
		if (
			...
			punchJobs = await punchData.json();
			// issueBD = await issueBurndown.json();
			hydroJobs = await hydroData.json();
...
<div class="finish-container">
	...
	{:else}
		...
		<!-- Department tables -->
		<div class="card-view">
			<Dept parts={punchJobs} />
			<Dept parts={hydroJobs} />
		</div>
	{/if}
</div>
