...
<script>
	...
	async function filterWT() {
		let primeWT = await primerParts
			.filter((part) => part.COMMENTS != null)
			.filter((c) => c.COMMENTS.includes('WET'));
		let tcWT = await topCoatParts
			.filter((part) => part.COMMENTS != null)
			.filter((c) => c.COMMENTS.includes('WET'));
		wetTest = [...primeWT, ...tcWT];
	}

	// this works
	// console.log(
	// 	primerParts.filter((part) => part.COMMENTS != null).filter((c) => c.COMMENTS.includes('WET'))
	// );
	onMount(() => {
		filterWT();
